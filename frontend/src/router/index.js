...
const routes = [
  ...
  },
  {
    ...
    name: 'GestionTramite',
    component: () => import('../views/funcionario/GestionTramiteView.vue'),
    meta: { requiresAuth: true, rol: 'funcionario' },
    props: true
  },
  // ─── Ruta compartida ──────────────────────────────────────────────
  {
    path: '/tramites/:id',
    name: 'DetalleTramite',
    component: () => import('../views/DetalleTramiteView.vue'),
    meta: { requiresAuth: true },
    props: true
  },
]
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.meta.rol && auth.user?.rol !== to.meta.rol) {
    ...
    if (auth.user?.rol === 'funcionario') return next({ name: 'FuncionarioDashboard' })
  }
